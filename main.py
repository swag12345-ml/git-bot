...
class FinancialFlows:
    ...
    def fetch_live_market_data(tickers: List[str]) -> Dict[str, Any]:
        """Fetch live market data from Yahoo Finance with 1-hour cache."""
        market_data = {}
    ...
    @staticmethod
    def demo_dashboard():
        """Advanced Real-Time Market Dashboard with live data."""
        if not TEST_MODE:
            st.markdown('<div class="flow-card"><h2>🌍 Advanced Real-Time Market Dashboard</h2><p>Live market data with interactive analytics, portfolio insights, and detailed fundamentals powered by Yahoo Finance.</p></div>', unsafe_allow_html=True)

            ticker_groups = {
                "US & Global": ["^GSPC", "^IXIC", "^DJI"],
                "India": ["^NSEI", "^BSESN"],
            ...
            tickers = ticker_groups[selected_group].copy()

            custom_tickers = st.text_input(
                "➕ Add custom tickers (comma-separated, e.g., AAPL, TSLA, MSFT)",
                placeholder="Enter additional ticker symbols..."
            )
            ...
            with st.spinner("Fetching live market data..."):
                market_data = FinancialFlows.fetch_live_market_data(tickers)
                ...
                with cols[idx % 5]:
                    if 'error' not in data:
                        ...
                        <div style="
                            ...
                            text-align: center;
                        ">
                            <h4 style="color: #ffffff; margin: 0; font-size: 0.9rem;">{data['name']}</h4>
                            <h2 style="color: #ffffff; margin: 0.5rem 0; font-size: 1.5rem;">${data['current_price']:,.2f}</h2>
                            <p style="color: {color}; margin: 0; font-size: 1rem; font-weight: bold;">{arrow} {change:+.2f}%</p>
                        </div>
                        ''', unsafe_allow_html=True)
                    else:
            ...
            fig_portfolio = go.Figure(data=[go.Pie(
                ...
                hole=0.4,
                marker=dict(colors=['#3b82f6', '#f59e0b', '#8b5cf6', '#10b981', '#6b7280']),
            ...
            fig_portfolio.update_layout(
                title={
                    'text': '💰 Live Portfolio Allocation ($10,000)',
                    'x': 0.5,
                    'xanchor': 'center',
            ...
            st.markdown("---")
            st.markdown("### 📊 5-Day Market Trend Overview")

            fig_trends = go.Figure()

            for ticker, data in market_data.items():
                if 'error' not in data and data['history']:
                    fig_trends.add_trace(go.Scatter(
                        x=list(range(len(data['history']))),
                        y=data['history'],
                        mode='lines+markers',
                        name=data['name'],
                        line=dict(width=3),
            ...
            fig_trends.update_layout(
                title={
                    'text': '📈 Multi-Asset Price Trends (Last 5 Days)',
                    'x': 0.5,
                    'xanchor': 'center',
                    'font': {'size': 18, 'color': 'white'}
                },
            ...
            with st.expander("📋 Detailed Fundamentals & Market Info", expanded=False):
                for ticker, data in market_data.items():
                    if 'error' not in data:
                        ...
                        with col2:
                            ...
                            st.metric("Daily Change", f"{data['change_pct']:+.2f}%", delta_color=change_color)
                        with col3:
