...
pip install streamlit plotly pandas numpy python-dotenv langchain-groq PyMuPDF xhtml2pdf
"""
...
class FinancialFlows:
    ...
    @staticmethod
    def financial_document_analyzer():
        """Financial Document Analyzer flow with PDF processing."""
        if not TEST_MODE:
            st.markdown('<div class="flow-card"><h2>📄 Financial Document Analyzer</h2><p>Upload your financial documents to extract insights and generate comprehensive reports.</p></div>', unsafe_allow_html=True)

        uploaded_file = None
        if not TEST_MODE:
            uploaded_file = st.file_uploader("Upload Financial PDF", type=['pdf'], help="Upload bank statements, salary slips, or investment summaries")
        ...
        if uploaded_file:
            with st.spinner("Extracting data from PDF..."):
                ...
                financial_data = analyze_financial_text(text)
                ...
                if not TEST_MODE:
                    ...
                    with col1:
                        st.markdown(display_metric_card(
                            "Total Income",
                            f"₹{financial_data['total_income']:,.2f}"
                        ), unsafe_allow_html=True)

                    with col2:
                        st.markdown(display_metric_card(
                            "Total Expenses",
                            f"₹{financial_data['total_expenses']:,.2f}"
                        ), unsafe_allow_html=True)

                    with col3:
                        st.markdown(display_metric_card(
                            "Total Investments",
                            f"₹{financial_data['total_investments']:,.2f}"
                        ), unsafe_allow_html=True)

                    with col4:
                        st.markdown(display_metric_card(
                            "Total Debt",
                            f"₹{financial_data['total_debt']:,.2f}"
                        ), unsafe_allow_html=True)
                    ...
                    with col1:
                        st.markdown(display_metric_card(
                            "Net Savings",
                            f"₹{financial_data['net_savings']:,.2f}",
                            color='green' if financial_data['net_savings'] > 0 else 'red'
                        ), unsafe_allow_html=True)

                    with col2:
                        st.markdown(display_metric_card(
                            "Savings Rate",
                            f"{financial_data['savings_rate']:.1f}%"
                        ), unsafe_allow_html=True)

                    st.subheader("📈 Financial Visualization")
                    fig = show_visual_summary(financial_data)
                    ...
                    with st.spinner("Generating AI insights..."):
                        ...
                        insights = generate_ai_insights_for_document(financial_data, llm_instance)

                        st.markdown(f'''
                        <div class="ai-suggestions-card">
                            <h4>💡 Personalized Financial Insights</h4>
                    ...
                    if st.button("Generate PDF Report", type="primary"):
                        with st.spinner("Creating PDF report..."):
                            pdf_buffer = generate_pdf_report(financial_data, insights)
                            st.download_button(
...
def analyze_financial_text(text):
    """Analyze financial text and extract key metrics using regex."""
    data = {
        'total_income': 0,
        'total_expenses': 0,
        'total_investments': 0,
    ...
    income_patterns = [
        r'(?:salary|income|earnings|revenue|wages|pay)[\s:]*₹?\s*([\d,]+(?:\.\d{2})?)',
        r'(?:credit|deposit)[\s:]*₹?\s*([\d,]+(?:\.\d{2})?)',
    ]

    expense_patterns = [
        r'(?:expense|expenditure|spending|cost|payment|debit|withdrawal)[\s:]*₹?\s*([\d,]+(?:\.\d{2})?)',
        r'(?:rent|utilities|groceries|bills)[\s:]*₹?\s*([\d,]+(?:\.\d{2})?)',
    ]

    investment_patterns = [
        r'(?:investment|mutual fund|stocks|equity|bonds|securities|portfolio)[\s:]*₹?\s*([\d,]+(?:\.\d{2})?)',
        r'(?:SIP|FD|fixed deposit)[\s:]*₹?\s*([\d,]+(?:\.\d{2})?)',
    ]

    debt_patterns = [
        r'(?:loan|debt|EMI|liability|outstanding|balance due)[\s:]*₹?\s*([\d,]+(?:\.\d{2})?)',
        r'(?:credit card|mortgage|borrowing)[\s:]*₹?\s*([\d,]+(?:\.\d{2})?)',
    ]
    ...
    for pattern in income_patterns:
        ...
        for match in matches:
            ...
            if value > 0:
                data['income_items'].append(value)
    ...
    for pattern in expense_patterns:
        ...
        for match in matches:
            value = float(match.group(1).replace(',', ''))
            if value > 0:
                data['expense_items'].append(value)
                data['total_expenses'] += value
    ...
    for pattern in investment_patterns:
        ...
        for match in matches:
            value = float(match.group(1).replace(',', ''))
            if value > 0:
                data['investment_items'].append(value)
                data['total_investments'] += value
    ...
    for pattern in debt_patterns:
        ...
        for match in matches:
            value = float(match.group(1).replace(',', ''))
            if value > 0:
                data['debt_items'].append(value)
                data['total_debt'] += value

    data['net_savings'] = data['total_income'] - data['total_expenses']

    if data['total_income'] > 0:
        data['savings_rate'] = (data['net_savings'] / data['total_income']) * 100
    else:
        data['savings_rate'] = 0
...
def show_visual_summary(data):
    """Generate Plotly visualization for financial data."""
    categories = []
    values = []

    if data['total_income'] > 0:
        categories.append('Income')
        values.append(data['total_income'])

    if data['total_expenses'] > 0:
        categories.append('Expenses')
        values.append(data['total_expenses'])

    if data['total_investments'] > 0:
        categories.append('Investments')
        values.append(data['total_investments'])

    if data['total_debt'] > 0:
        categories.append('Debt')
        values.append(data['total_debt'])
    ...
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=('Financial Overview', 'Distribution'),
    ...
    fig.add_trace(
        go.Pie(
            labels=categories,
            values=values,
            marker_colors=['#2ecc71', '#e74c3c', '#3498db', '#f39c12']
        ),
        row=1, col=2
    )
...
def generate_ai_insights_for_document(data, llm_instance):
    """Generate AI insights for financial document using existing AI model."""
    if not llm_instance:
        return f"""
Based on the extracted financial data:

• Your total income is ₹{data['total_income']:,.2f}
• Your total expenses are ₹{data['total_expenses']:,.2f}
• Your net savings are ₹{data['net_savings']:,.2f} ({data['savings_rate']:.1f}% savings rate)
• Your total investments are ₹{data['total_investments']:,.2f}
• Your total debt is ₹{data['total_debt']:,.2f}

Key Recommendations:
1. {'Excellent savings rate! Consider increasing investments.' if data['savings_rate'] > 20 else 'Try to increase your savings rate to at least 20%.'}
2. {'Focus on reducing debt through strategic repayment.' if data['total_debt'] > 0 else 'Great job maintaining zero debt!'}
3. {'Consider diversifying your investment portfolio.' if data['total_investments'] > 0 else 'Start investing to build wealth over time.'}
4. Review and optimize your expense categories to improve savings.
"""

    try:
        prompt = f"""
You are a financial advisor analyzing a client's financial document.
...
Please provide:
1. A brief assessment (2-3 sentences) of their financial health
2. 3-5 specific, actionable recommendations to improve their financial situation
3. Priority areas they should focus on
...
def generate_pdf_report(data, insights):
    """Generate HTML-based PDF report using xhtml2pdf."""
    html = f"""
    <html>
    <head>
        <style>
    ...
    <body>
        ...
        <table>
            ...
            <tr>
                <td>Total Income</td>
                <td>{data['total_income']:,.2f}</td>
            </tr>
            <tr>
                <td>Total Expenses</td>
                <td>{data['total_expenses']:,.2f}</td>
            </tr>
            <tr>
                <td>Total Investments</td>
                <td>{data['total_investments']:,.2f}</td>
            </tr>
            <tr>
                <td>Total Debt</td>
                <td>{data['total_debt']:,.2f}</td>
            </tr>
            <tr>
                <td><strong>Net Savings</strong></td>
                <td><strong>{data['net_savings']:,.2f}</strong></td>
            </tr>
            <tr>
                <td><strong>Savings Rate</strong></td>
                <td><strong>{data['savings_rate']:.1f}%</strong></td>
            </tr>
        </table>
        ...
        <div class="footer">
            <p>Generated by AI Financial Advisor - LLAMA 3.3</p>
...
def main():
    ...
    menu = st.sidebar.selectbox(
        "Choose Section",
        ["Demo Dashboard", "Budgeting", "Investments", "Debt", "Retirement", "📄 Financial Document Analyzer"]
    )
    ...
    elif st.session_state.selected_page == "📄 Financial Document Analyzer":
        FinancialFlows.financial_document_analyzer()
