    ...
    def investing_flow():
        ...
        # STEP 2: Risk Profiling with live gauge
            ...
            if not TEST_MODE:
                ...
                fig_risk = go.Figure(go.Indicator(
                    mode="gauge+number",
                    value=risk_score,
                    title={'text': f"<b>Risk Score: {risk_profile}</b>", 'font': {'size': 18, 'color': 'white'}},
                    number={'font': {'size': 36, 'color': 'white'}},
                    gauge={
        ...
        # STEP 3: Asset Sandbox with sliders
            ...
            if not TEST_MODE:
                ...
                if total_alloc != 100:
                    st.warning(f"Total allocation: {total_alloc}%. Please adjust to equal 100%.")
                else:
        ...
        # STEP 4: Review & AI Analysis
            ...
            if not TEST_MODE:
                ...
                # AI Analysis button
                if st.button("🔍 Analyze My Portfolio", type="primary", key="analyze_ai"):
                ...
                if st.button("💾 Save Profile", key="save_profile"):
                    ...
                    st.success(f"Profile saved! Total saved profiles: {len(st.session_state.saved_profiles)}")

                # Display saved profiles
                if st.session_state.saved_profiles:
                    st.markdown("### Saved Profiles")
                    for i, profile in enumerate(st.session_state.saved_profiles):
                        with st.expander(f"Profile {i+1} - {profile['timestamp']} ({profile['risk_profile']})"):
                            st.json(profile)
